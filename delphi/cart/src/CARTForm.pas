...
uses
  ...
  Dialogs, ExtCtrls, ComCtrls, Menus, ImgList, ToolWin, ActnList, StdCtrls;
...
type
  ...
  public
    ...
    procedure FullPaint;
    procedure PaintCanvas(Canvas : TCanvas; Sc,Gc : Integer);
...
begin
 with CellA.CARul do
 begin
   InvBtn.Enabled := IsRev and CheckCap(capInvert);
...
begin
  Result := False;
  if FileExists(ChangeFileExt(PalName,'.ccl')) then
...
begin
 ...
 begin
  Width := Xc*(Sc+Gc)+1;
  Height := Yc*(Sc+Gc)+1;
  PaintCanvas(Canvas,Sc,Gc)
 end;
end;

procedure TFormCA.PaintCanvas(Canvas: TCanvas; Sc, Gc: Integer);
var
 i,j : integer;
begin
 with Canvas do
 begin
  Brush.Color := BkCol;
  FillRect(Rect(0,0,Xc*(Sc+Gc)+1,Yc*(Sc+Gc)+1));
  for i := 0 to Xc-1 do
   for j := 0 to Yc-1 do
   begin
...
begin
 ...
 begin
  ...
  begin
    ...
    PaintCanvas(FormPix.Pix.Canvas,Sz,Gp);
 ...
 begin
  ...
  begin
   ...
   begin
    ...
    if FileExists(PalName+'.mcp') then
   ...
   begin
    ...
    begin
     ...
     begin
      ...
      RuleName := ChangeFileExt(ExtractFileName(FileName),'');
      if FileExists(ChangeFileExt(FileName,'.colors')) then
